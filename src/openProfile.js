...
const poolName = "/amazom_1";
const pathName = "C:/System/Profiles";
...
export class GoLogin {
  ...
  async getProfile(profilepath) {
    console.log("profile feteched from database")
    //console.log(profileResponse.body)
        //    let ref=fire_db.ref("profile_1");
        ...
        let proxy = fs.readFileSync(path.join(profilepath, '/../../proxy.json'))
        proxy = JSON.parse(proxy)
        let data = fs.readFileSync(path.join(profilepath + '/config.json'), 'utf8');
        data = JSON.parse(data)

        if(proxy.useGlobleProxy)
        {
          if(proxy.proxyEnabled)
          {
            if(proxy.useRandomProxy)
            {
                let proxyNo = between(0,4)
                data.proxyEnabled = proxy.proxyEnabled
                data.proxy = proxy["proxy_" + proxyNo]
            ...
            {
              data.proxyEnabled = proxy.proxyEnabled
              data.proxy = proxy["proxy_"+proxy.staticProxyNo]
   ...
   //console.log("profile feteched from database" + data)     
  	return data;
  }
  ...
  async createStartup(local=false) {
   ...
   // await rimraf(profilePath, () => null);
    ...
    profile = await this.getProfile(profilePath);
  ...
  async startRemote(delay_ms = 10000) {
    ...
    */
    ...
    // if (profileResponse.body === 'ok') {
        const profile = await this.getProfile(profilePath);
